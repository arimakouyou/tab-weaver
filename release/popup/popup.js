/**
 * TabList Chrome Extension - Main Popup Controller
...
class TabListController {
    ...
    showError(message, details = '') {
        ...
        errorElement.innerHTML = `
            ...
            <button class="error-retry" onclick="window.tabListController.refreshTabData()">
                🔄 再試行
            </button>
        `;
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        // 初期化の開始ログ
        // Initializing TabList Controller
        
        window.tabListController = new TabListController();
        
        // 初期化完了の待機
        if (window.tabListController.initializationPromise) {
            await window.tabListController.initializationPromise;
        }
        
        // TabList Controller initialized
        
    } catch (error) {
        console.error('Failed to initialize TabList Controller:', error);
...
window.addEventListener('error', (event) => {
    console.error('TabList Global Error:', {
    ...
    // ユーザーへの通知
    if (window.tabListController && window.tabListController.clipboardManager) {
        window.tabListController.clipboardManager.showCopyFeedback(
...
window.addEventListener('unhandledrejection', (event) => {
    console.error('TabList Unhandled Promise Rejection:', {
        reason: event.reason,
